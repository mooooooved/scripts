...
Write-Host "====      INSTALLING DUNGEON CRAWL STONE SOUP      ===="
Write-Host "====         you can minimize this window          ===="
Write-Host "====           brought to you by rretroo           ===="

$dlPath = "$HOME\Downloads"
$ioq3Path = "$dlPath\DCSS"

If (-Not (Test-Path $ioq3Path)) {
  New-Item -ItemType Directory -Path $ioq3Path
}

Download-File -Url "https://github.com/crawl/crawl/releases/download/0.29.1/dcss-0.29.1-win32-console.zip" -Path "$ioq3Path\crawl.zip"

Expand-Archive -LiteralPath "$ioq3Path\crawl.zip" -DestinationPath $ioq3Path
